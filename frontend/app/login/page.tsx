...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-[calc(100svh-3.5rem)] bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.08),_transparent_28%),linear-gradient(180deg,_rgba(255,255,255,0.96),_rgba(240,253,250,0.86)_52%,_rgba(255,255,255,1))] px-4 py-16 dark:bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.10),_transparent_28%),linear-gradient(180deg,_rgba(10,10,10,1),_rgba(8,18,14,1)_52%,_rgba(10,10,10,1))] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-md">
        ...
        >
          ...
          {error && (
            <div className="mt-6 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-600 dark:text-red-400">
              {error}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <motion.button
              type="button"
              onClick={() => void handle(() => signInWithGoogle("employer"))}
              disabled={loading}
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.99 }}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border/50 bg-background text-sm font-medium text-foreground transition hover:bg-foreground/5 disabled:opacity-60"
            >
            ...
            <motion.button
              type="button"
              onClick={() => void handle(() => signInWithGitHub("freelancer"))}
              disabled={loading}
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.99 }}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border/50 bg-background text-sm font-medium text-foreground transition hover:bg-foreground/5 disabled:opacity-60"
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            No account?{" "}
            <Link href="/signup" className="font-medium text-green-600 transition hover:text-green-700 dark:text-green-400">
              Sign up
            </Link>
          </p>
        </motion.div>
